    ...
    for j, part in enumerate(row_parts):
        ...
        if part_start != -1:
            if abs(symbol_index - part_start) <= 1 or abs(symbol_index - part_end) <= 1:
                ...
                if possible_gears.get(f"{current_row}_{symbol_index}"):
                    possible_gears[f"{current_row}_{symbol_index}"].append(int(part))
                else:
                    possible_gears[f"{current_row}_{symbol_index}"] = [int(part)]
...
confirmed_gears = []
for each in possible_gears.values():
    if len(each) == 2:
        confirmed_gears.append(each[0] * each[1])
        
print(f"gears are {sum(confirmed_gears)}")